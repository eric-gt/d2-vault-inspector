...
------------ | ------------- | ------------- | -------------
**DisplayProperties** | [***AllOfDestinyDefinitionsDestinySandboxPerkDefinitionDisplayProperties**](AllOfDestinyDefinitionsDestinySandboxPerkDefinitionDisplayProperties.md) | These display properties are by no means guaranteed to be populated. Usually when it is, it&#x27;s only because we back-filled them with the displayProperties of some Talent Node or Plug item that happened to be uniquely providing that perk. | [optional] [default to null]
...
**DamageType** | **int32** | If this perk grants a damage type to a weapon, the damage type will be defined here.  Unless you have a compelling reason to use this enum value, use the damageTypeHash instead to look up the actual DestinyDamageTypeDefinition. | [optional] [default to null]
